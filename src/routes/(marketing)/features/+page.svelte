  ...
  const competitors = [
      {
          ...
          advantages: [
              `${mpmf.hivey_competitor_features_1()}`,
              `${mpmf.hivey_competitor_features_2()}`,
              `${mpmf.hivey_competitor_features_3()}`
          ]
      },
      {
          ...
          advantages: [
              `${mpmf.hivey_competitor_features_1()}`,
              `${mpmf.hivey_competitor_features_4()}`,
              `${mpmf.hivey_competitor_features_5()}`
          ]
      },
      {
          name: "Rustpad, Codeshare",
          advantages: [
              `${mpmf.hivey_competitor_features_6()}`,
              `${mpmf.hivey_competitor_features_7()}`,
              `${mpmf.hivey_competitor_features_8()}`
          ]
      }
  ];
...
<svelte:head>
  <title>{mpm.features()} | {m.app_name()}</title>
  <meta name="description" content={`${mpm.features()} for ${m.app_name()}`} />
</svelte:head>
...
<div class="max-w-6xl mx-auto px-6 md:px-8">
  ...
  <section class="my-12" in:fade={{ duration: 300}}>
    <h2 class="text-3xl font-bold mb-6 text-center">{mpmf.overview_of_hivey()}</h2>
    <p class="text-lg text-center text-muted-foreground max-w-3xl mx-auto">
      {mpm.one_editor_all_formats_description()}
    </p>
  ...
  <section id="why-use" class="my-12"  in:fade={{ duration: 300 }}>
    <h2 class="text-3xl font-bold mb-6">{mpmf.why_use_hivey()}</h2>
  ...
  <section id="cta" class="my-12 py-16relative overflow-hidden bg-gradient-to-br from-primary/60 to-primary/20 bg-cover shadow-xl rounded-2xl" in:fade={{ duration: 300 }}>
    ...
    <div class="max-w-4xl mx-auto text-center py-16 px-6 relative z-10">
      <h2 class="text-3xl md:text-4xl font-bold mb-4">{mpmf.try_it_now()}</h2>
            
      <p class="text-xl mb-10 max-w-4xl mx-auto">
        {mpmf.try_it_now_description()}
      </p>
